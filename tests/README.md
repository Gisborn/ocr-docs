...
```bash
# Как в CI — с образами GHCR :latest (проверяет то, что уже запушено)
make e2e-ci
# или
./scripts/e2e-test-ci.sh

# С локально собранным кодом (проверяет текущие изменения)
make e2e-local
# или
...
**Разница:**
- `e2e-ci` — тянет `:latest` из GHCR, как делает CI перед деплоем
- `e2e-local` — собирает образы из текущего кода и тестирует их

#### Ручной запуск (если сервисы уже подняты)
